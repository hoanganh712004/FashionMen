...
public class CheckOutFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        // Casting http servlet
        HttpServletRequest requestHttp = (HttpServletRequest) request;
        HttpServletResponse responseHttp = (HttpServletResponse) response;
        HttpSession session = requestHttp.getSession();

        // Create object Account from SESSION_ACCOUNT
        User account = (User) session.getAttribute("account");
        List<CartItemDTO> cartItemList = (List<CartItemDTO>) session.getAttribute("cartItemList");

        if (account == null) {
            responseHttp.sendRedirect(requestHttp.getContextPath() + "/views/common/user/login.jsp");
        } else {
            if (cartItemList.isEmpty()) {
                 responseHttp.sendRedirect(requestHttp.getContextPath() + "/home");
            }
        }
